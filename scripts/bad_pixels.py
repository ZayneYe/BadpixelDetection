...
def generate_bad_pixel(value, delta):
    if (1 + delta) * value > 1023:
        delta_range = [(0, int((1 - delta) * value))]
    else:
        delta_range = [(0, int((1 - delta) * value)), (int((1 + delta) * value), 1023)]
...
if __name__ == "__main__":
    delta = 0.7
    bad_rate = 0.01
    data_dir = f'/data1/Bad_Pixel_Detection/data/Invertible_ISP_{delta}_{bad_rate}'
    ...
    for i, dng in enumerate(os.listdir(org_dir)):
        ...
        pixels_to_modify = int(0.7 * total_pixels)
        ...
        bad_pixel_values = np.vectorize(generate_bad_pixel)(current_pixel_values, delta)
